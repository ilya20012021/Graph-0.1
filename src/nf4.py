...
def nf4():
    s = open("nf3.txt", "r").readlines()
    v = "".join(s)
    u = v.split()
    for i in u:
        print("Файл: ", i)
    print('Введите команду "NF4 NODE <filename>"')
    k5 = [str(i) for i in input().split()]
    if "NF4" in k5 and "NODE" in k5 and len(k5) == 3 and k5[2] in u:
        print("4 НФ - когда таблица для конвертации в граф находится в 3 НФ и в ней содержится первичный ключ исходной таблицы.")
